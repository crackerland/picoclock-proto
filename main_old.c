...
void command(char *c)
{
  ...
  if (strstr(left, "narkose"))
  {
    QMI8658_enableWakeOnMotion();
  }
  if (strstr(left, "qmiinit"))
  {
    QMI8658_init();
  }
  if (strstr(left, "qmireset"))
  {
    QMI8658_reset();
  }
  if (strstr(left, "scrs"))
...
int main(void)
{
  ...
  // PicoBattery bat1 = {"GOOD\0", 1100.0f, 4.19f, 3.346729f, 4.189453f, 0.0f, 0.0f}; // 1100ma
...
#define ACCMAX 500.0f
    ...
    if (fire_pressed)
    {
      ...
      if ((button0_dif || button1_dif) && (plosa->editpos == EPOS_CENTER))
      { // only from central position (flag)
    ...
    ++resulti;
    resulti &= 0x0f;
    result[resulti] = (*plosa->bat.Base.Read)(&plosa->bat.Base); // read_battery();
    ...
    if (cmode == CM_Changepos)
    {
      if (NO_POS_MODE)
      {
        if (dir_x == D_PLUS)
        {
          if (pos_matrix_x < positions[plosa->theme]->dim_x - 1)
            ++pos_matrix_x;
        }
        if (dir_x == D_MINUS)
        {
        ...
        if (dir_y == D_PLUS)
        {
          if (pos_matrix_y < positions[plosa->theme]->dim_y - 1)
            ++pos_matrix_y;
        }
        if (dir_y == D_MINUS)
        {
